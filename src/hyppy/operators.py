...
This module provides functions for calculating the operators. It includes
functions for the single-spin operators as well as many-spin superoperators.
"""
...
def op_E(S: float) -> np.ndarray:
    """
    Generates the unit operator of quantum number `S`.
    ...
    -------
    E : numpy.ndarray
        NumPy array corresponding to the E-operator.
    """

    # Generate an unit operator of correct dimension
    dim = int(2*S+1)
    E = np.eye(dim)
...
def op_Sx(S: float) -> np.ndarray:
    """
    Generates the spin operator Sx of quantum number `S`.
    ...
    -------
    Sx : numpy.ndarray
        NumPy array corresponding to the x-operator.
    """
    # Calculate Sx using raising and lowering operators
...
def op_Sy(S: float) -> np.ndarray:
    """
    Generates the spin operator Sy of quantum number `S`.
    ...
    -------
    Sx : numpy.ndarray
        NumPy array corresponding to the y-operator.
    """
    # Calculate Sy using raising and lowering operators
...
def op_Sz(S: float) -> np.ndarray:
    """
    Generates the spin operator Sz of quantum number `S`.
    ...
    -------
    Sz : numpy.ndarray
        NumPy array corresponding to the z-operator.
    """
    ...
    # Go over the possible states m
    for i in range(len(m)):

        # Set the diagonal element to m
        Sz[i, i] = m[i]  
...
def op_Sp(S: float) -> np.ndarray:
    """
    Generates the spin raising operator of quantum number `S`.
    ...
    -------
    Sp : numpy.ndarray
        NumPy array corresponding to the raising operator.
    """
    ...
    # Go over the possible states m, avoiding the highest state, which cannot be raised further
...
def op_Sm(S: float) -> np.ndarray:
    """
    Generates the spin lowering operator of quantum number `S`.
    ...
    -------
    Sm : numpy.ndarray
        NumPy array corresponding to the lowering operator.
    """
    # Get the possible spin magnetic quantum numbers
    m = np.arange(-S, S+1)
    ...
    # Go over the possible states m, avoiding the lowest state, which cannot be lowered further
...
def op_T(S: float, l: int, q: int) -> np.ndarray:
    """
    Numerical spherical tensor operator of spin quantum number `S`, rank `l` and projection `q`.
    ...
    This function is called often. Cached for high performance.
    ...
    -------
    T : numpy.ndarray
        NumPy array corresponding to the spherical tensor operator.
    """
    # Calculate the operator with maximum projection q=l
    T = (-1)**l * 2**(-l/2) * np.linalg.matrix_power(op_Sp(S), l)

    # Perform the necessary amount of lowerings
    for i in range(l - q):
...
def op_T_coupled(l:int, q:int, l1:int, s1:float, l2:int, s2:float) -> np.ndarray:
    """
    Calculates the coupled irreducible spherical tensor of rank `l` and projection `q`
    ...
    # Go over the projections
    for q1 in range(-l1, l1+1):
        for q2 in range(-l2, l2+1):

            # Use coupling of angular momenta equation
...
def structure_coefficients(spin: float, side: str) -> np.ndarray:
    """
    Generates the (normalized) structure coefficients of the operator algebra
    of a single spin. These are used in constructing the product superoperators.
    ...
    for j in range(mult**2):
        ...
        for k in range(mult**2):
            ...
            # Take care of the normalisation
...
def sop_E(spin_system: SpinSystem) -> csc_array:
    ...
    -------
    unit : csc_array
        An array corresponding to the unit operator.
    """

    # Make the unit operator
...
def op_P(op_def: tuple, spins: tuple, include_unit: bool=True) -> np.ndarray:
    """
    Produces a product operator defined by `op_def` in the Zeeman eigenbasis.
    ...
    include_unit : bool
        Specifies whether the unit operators are taken into account in the
        product operator.
...
def sop_P(spin_system: SpinSystem, op_def: tuple, side: str) -> csc_array:
    """
    Produces a product superoperator that corresponds to the product operator
    defined by `op_def`.
    ...
    # Make empty lists for storing non-zero structure coefficients and their indices
    c_jk = []
    j = []
    k = []
    ...
    # Initialize an empty list for the superoperator values and indices
    sop_vals = []
    sop_j = []
    sop_k = []
    ...
    for m in range(prod_c_jk.shape[0]):
        ...
        if j_op.shape[0] != 0 or k_op.shape[0] != 0:
            ...
            # Operator definitions must match in order for the product of structure coefficients to be nonzero
...
def superoperator(spin_system: SpinSystem, operators: Union[str, list], spins: Union[int, list], side: str='comm') -> csc_array:
    """
    Generates the requested superoperator for the given spins.
    ...
    operators : str or list
        ...
        - list :
            ...
            generated. Must match the length of `spins`.
            For example: ['I_+', 'I_-']
    spins : int or list
...
def sop_T_coupled(spin_system: SpinSystem, l: int, q: int, spin_1: int, spin_2: int=None) -> csc_array:
    """
    Calculates the product superoperator that corresponds to the coupled spherical tensor
    operator of rank `l` and projection `q` from two spherical tensor operators of rank 1.
    ...
    spin_2 : int
        Index of the second spin. Left empty for linear single-spin interactions (shielding).
    ...
    # Process single-spin quadratic and two-spin bilinear interactions
    if isinstance(spin_2, int):
        ...
        for q1 in range(-1, 2):
            for q2 in range(-1, 2):
                ...
                # Use coupling of angular momenta equation
                sop += la.CG_coeff(1, q1, 1, q2, l, q) * sop_P(spin_system, op_def, 'comm')

    # Process linear single-spin interactions differently
    else:
        ...
        for q1 in range(-1, 2):
            ...
            # Use coupling of angular momenta equation
            sop += la.CG_coeff(1, q1, 1, 1, l, q) * sop_P(spin_system, op_def, 'comm')
