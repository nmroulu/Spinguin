...
def empty_state(spin_system: SpinSystem) -> np.ndarray | sp.lil_array:
    """
    Return an empty Liouville-space state vector. Used to initialise state
    vectors before populating them with specific coefficients.

    Parameters
    ----------
    spin_system : SpinSystem
        Spin system for which the empty state is created.

    Returns
    -------
    rho : ndarray or lil_array
        Empty state vector.
    """
    ...
    if parameters.sparse_state:
        return sp.lil_array((spin_system.basis.dim, 1), dtype=complex)
    else:
        return np.zeros((spin_system.basis.dim, 1), dtype=complex)
...
def alpha_state(
    ...
    """
    Generate the alpha (spin-up) state for a selected spin-1/2 nucleus.
...
def beta_state(
    ...
    """
    Generate the beta (spin-down) state for a selected spin-1/2 nucleus.
