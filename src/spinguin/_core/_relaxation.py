...
def Perrin_integrals(
    ...
    integrand_P = (
        1
        / np.sqrt((a_x**2 + s) ** 3 * (a_y**2 + s) * (a_z**2 + s))
        * jacobian
    )
    integrand_Q = (
        1
        / np.sqrt((a_y**2 + s) ** 3 * (a_z**2 + s) * (a_x**2 + s))
        * jacobian
    )
    integrand_R = (
        1
        / np.sqrt((a_z**2 + s) ** 3 * (a_x**2 + s) * (a_y**2 + s))
        * jacobian
    )
        ...
        for target, gamma in enumerate(spin_system.gammas):
            ...
            if not spin_system.gammas[quad] == gamma:
                ...
                R1[target] += (
                    (J**2) * S * (S + 1) / 3
                    * (2 * T2)
                    / (1 + (omega_target - omega_quad)**2 * T2**2)
                )
                R2[target] += (
                    (J**2) * S * (S + 1) / 3
                    * (
                        T1
                        + T2 / (1 + (omega_target - omega_quad)**2 * T2**2)
                    )
                )
...
def _validate_relaxation_inputs(
    spin_system: SpinSystem,
) -> None:
    """
