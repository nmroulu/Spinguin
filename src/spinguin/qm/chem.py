...
def _dissociate_index_map(basis_A_bytes: bytes,
                          ...
                          spin_map_B_bytes: bytes) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
...
def dissociate_index_map(basis_A: np.ndarray,
                         ...
                         spin_map_B: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """
    Generates arrays that map the state indices from the basis set C 
    to basis sets A and B. This function is used in `dissociate()`.

    Example. Basis set C contains five spins, which are indexed as
    (0, 1, 2, 3, 4). We want to dissociate this into two subsystems
    A and B. Spins 0 and 2 should go to subsystem A and the rest to
    subsystem B. In this case, we define the following spin maps:
    ...
    ----------
    basis_A : ndarray
        Basis set for the subsystem A. It is a 2-dimensional array
        containing sequences of integers describing the Kronecker
        products of irreducible spherical tensors.
    basis_B : ndarray
        Basis set for the subsystem B. It is a 2-dimensional array
        containing sequences of integers describing the Kronecker
        products of irreducible spherical tensors.
    basis_C : ndarray
        Basis set for the composite system C. It is a 2-dimensional
        array containing sequences of integers describing the
        Kronecker products of irreducible spherical tensors.
    spin_map_A : ndarray
    ...
    index_map_CA : ndarray
        Corresponding indices of the matching elements in C. The array
        length is equal to `index_map_A`.
    index_map_B : ndarray
        Indices of states in B that also appear in C.
    index_map_CB : ndarray
        Corresponding indices of the matching elements in C. The array
        length is equal to `index_map_B`.
    """
    ...
    index_map_A, index_map_CA, index_map_B, index_map_CB = _dissociate_index_map(basis_A_bytes, basis_B_bytes, basis_C_bytes, spin_map_A_bytes, spin_map_B_bytes)
...
def dissociate(basis_A: np.ndarray,
               ...
               spin_map_A: tuple,
               spin_map_B: tuple) -> tuple[np.ndarray | sp.csc_array, np.ndarray | sp.csc_array]:
    """
    Dissociates the density vector of composite system C into density
    vectors of two subsystems A and B in a chemical reaction C -> A + B.

    Example. Spin system C has five spins, which are indexed as
    (0, 1, 2, 3, 4). We want to dissociate this into two subsystems A
    and B. Spins 0 and 2 should go to subsystem A and the rest to
    subsystem B. In this case, we define the following spin maps:
    ...
    # Get index mappings
    idx_A, idx_CA, idx_B, idx_CB = dissociate_index_map(basis_A, basis_B, basis_C, spin_map_A, spin_map_B)
...
def associate_index_map(basis_A: np.ndarray,
                        ...
                        basis_C: np.ndarray,
                        spin_map_A: np.ndarray,
                        spin_map_B: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    ...
    Example. We have spin system A that has two spins and spin system B that
    has three spins. These systems associate to form a composite spin system C
    that has five spins that are indexed (0, 1, 2, 3, 4). Of these, spins (0, 2)
    are from subsystem A and (1, 3, 4) from subsystem B. We have to choose how
    the spin systems A and B will be indexed in spin system C by defining the
    spin maps as follows:
    ...
    index_map_A, index_map_B, index_map_C = _associate_index_map(basis_A_bytes, basis_B_bytes, basis_C_bytes, spin_map_A_bytes, spin_map_B_bytes)
...
def associate(basis_A: np.ndarray,
    ...
    idx_A, idx_B, idx_C = associate_index_map(basis_A, basis_B, basis_C, spin_map_A, spin_map_B)
...
def permutation_matrix(basis: np.ndarray, spin_map: np.ndarray, sparse: bool=True) -> np.ndarray | sp.csc_array:
    """
...
def permute_spins(basis: np.ndarray, rho: np.ndarray | sp.csc_array, spin_map: np.ndarray, sparse: bool=True) -> np.ndarray | sp.csc_array:
    """
