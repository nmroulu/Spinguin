...
def make_basis(spins: np.ndarray, max_spin_order: int):
    ...
    spins : ndarray
        ...
        spin system.
    max_spin_order : int
        Defines the maximum spin entanglement that is considered in the basis set.
    """
    ...
    if max_spin_order < 1:
        raise ValueError(f"'max_spin_order' must be at least 1.")
    if max_spin_order > nspins:
        raise ValueError(f"'max_spin_order' must not be larger than number of spins in the system.")
...
def truncate_basis_by_coherence(basis: np.ndarray, coherence_orders: list) -> list:
    """
