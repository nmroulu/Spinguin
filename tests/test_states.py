...
class TestStates(unittest.TestCase):
    ...
    def test_alpha(self):
        ...
        alpha1_sparse = alpha_state(spin_system, 0, sparse=True)
        alpha2_sparse = alpha_state(spin_system, 1, sparse=True)
        alpha1_dense = alpha_state(spin_system, 0, sparse=False)
        alpha2_dense = alpha_state(spin_system, 1, sparse=False)

        # Compare
        self.assertTrue(np.allclose(rho_to_zeeman(spin_system, alpha1_sparse), alpha1_zeeman))
        self.assertTrue(np.allclose(rho_to_zeeman(spin_system, alpha2_sparse), alpha2_zeeman))
        self.assertTrue(np.allclose(rho_to_zeeman(spin_system, alpha1_dense), alpha1_zeeman))
        self.assertTrue(np.allclose(rho_to_zeeman(spin_system, alpha2_dense), alpha2_zeeman))
    ...
    def test_beta(self):
        ...
        beta1_sparse = beta_state(spin_system, 0, sparse=True)
        beta2_sparse = beta_state(spin_system, 1, sparse=True)
        beta1_dense = beta_state(spin_system, 0, sparse=False)
        beta2_dense = beta_state(spin_system, 1, sparse=False)

        # Compare
        self.assertTrue(np.allclose(rho_to_zeeman(spin_system, beta1_sparse), beta1_zeeman))
        self.assertTrue(np.allclose(rho_to_zeeman(spin_system, beta2_sparse), beta2_zeeman))
        self.assertTrue(np.allclose(rho_to_zeeman(spin_system, beta1_dense), beta1_zeeman))
        self.assertTrue(np.allclose(rho_to_zeeman(spin_system, beta2_dense), beta2_zeeman))
    ...
    def test_singlet(self):
        ...
        singlet_sparse = singlet_state(spin_system, 0, 1, sparse=True)
        singlet_dense = singlet_state(spin_system, 0, 1, sparse=False)
        self.assertTrue(np.allclose(singlet_zeeman, rho_to_zeeman(spin_system, singlet_sparse)))
        self.assertTrue(np.allclose(singlet_zeeman, rho_to_zeeman(spin_system, singlet_dense)))
    ...
    def test_triplet_zero(self):
        ...
        triplet_zero_sparse = triplet_zero_state(spin_system, 0, 1, sparse=True)
        triplet_zero_dense = triplet_zero_state(spin_system, 0, 1, sparse=False)
        self.assertTrue(np.allclose(triplet_zero_zeeman, rho_to_zeeman(spin_system, triplet_zero_sparse)))
        self.assertTrue(np.allclose(triplet_zero_zeeman, rho_to_zeeman(spin_system, triplet_zero_dense)))
    ...
    def test_triplet_plus(self):
        ...
        triplet_plus_sparse = triplet_plus_state(spin_system, 0, 1, sparse=True)
        triplet_plus_dense = triplet_plus_state(spin_system, 0, 1, sparse=False)
        self.assertTrue(np.allclose(triplet_plus_zeeman, rho_to_zeeman(spin_system, triplet_plus_sparse)))
        self.assertTrue(np.allclose(triplet_plus_zeeman, rho_to_zeeman(spin_system, triplet_plus_dense)))
    ...
    def test_triplet_minus(self):
        ...
        triplet_minus_sparse = triplet_minus_state(spin_system, 0, 1, sparse=True)
        triplet_minus_dense = triplet_minus_state(spin_system, 0, 1, sparse=False)
        self.assertTrue(np.allclose(triplet_minus_zeeman, rho_to_zeeman(spin_system, triplet_minus_sparse)))
        self.assertTrue(np.allclose(triplet_minus_zeeman, rho_to_zeeman(spin_system, triplet_minus_dense)))
        ...
        for i in test_states:
            for j in test_states:
                for k in test_states:
                    ...
                    state_sparse = state(spin_system, f"I({i},0) * I({j},1) * I({k},2)", sparse=True)
                    state_dense = state(spin_system, f"I({i},0) * I({j},1) * I({k},2)", sparse=False)
                    ...
                    self.assertTrue(np.allclose(rho_to_zeeman(spin_system, state_sparse), state_zeeman))
                    self.assertTrue(np.allclose(rho_to_zeeman(spin_system, state_dense), state_zeeman))
    ...
    def test_unit_state(self):
        ...
        unit_dense = unit_state(spin_system, sparse=False, normalized=False)

        # Compare
        self.assertTrue(np.allclose(rho_to_zeeman(spin_system, unit_sparse), unit_zeeman))
        self.assertTrue(np.allclose(rho_to_zeeman(spin_system, unit_dense), unit_zeeman))

        # Create the trace-normalized unit state in the spherical tensor basis
        unit_sparse = unit_state(spin_system, sparse=True, normalized=True)
        unit_dense = unit_state(spin_system, sparse=False, normalized=True)
                    ...
                    for l in test_states:
                        for m in test_states:
                            for n in test_states:
                                ...
                                result_sparse = measure(spin_system, state_sparse, f"I({l},0) * I({m},1) * I({n},2)")
                                result_dense = measure(spin_system, state_dense, f"I({l},0) * I({m},1) * I({n},2)")
    ...
    def test_rho_thermal_equilibrium(self):
        ...
        for i in range(size):
            ...
            Mz_measured_sparse = measure(spin_system, rho_eq_sparse, f"I(z, {i})")
