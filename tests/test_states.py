...
class TestStates(unittest.TestCase):
    ...
    def test_alpha(self):
        ...
        alpha1_sparse = states.alpha(spin_system, 0, sparse=True)
        alpha2_sparse = states.alpha(spin_system, 1, sparse=True)
        alpha1_dense = states.alpha(spin_system, 0, sparse=False)
        alpha2_dense = states.alpha(spin_system, 1, sparse=False)

        # Compare
        self.assertTrue(np.allclose(states.rho_to_zeeman(spin_system, alpha1_sparse), alpha1_zeeman))
        self.assertTrue(np.allclose(states.rho_to_zeeman(spin_system, alpha2_sparse), alpha2_zeeman))
        self.assertTrue(np.allclose(states.rho_to_zeeman(spin_system, alpha1_dense), alpha1_zeeman))
        self.assertTrue(np.allclose(states.rho_to_zeeman(spin_system, alpha2_dense), alpha2_zeeman))
    ...
    def test_beta(self):
        ...
        beta1_sparse = states.beta(spin_system, 0, sparse=True)
        beta2_sparse = states.beta(spin_system, 1, sparse=True)
        beta1_dense = states.beta(spin_system, 0, sparse=False)
        beta2_dense = states.beta(spin_system, 1, sparse=False)

        # Compare
        self.assertTrue(np.allclose(states.rho_to_zeeman(spin_system, beta1_sparse), beta1_zeeman))
        self.assertTrue(np.allclose(states.rho_to_zeeman(spin_system, beta2_sparse), beta2_zeeman))
        self.assertTrue(np.allclose(states.rho_to_zeeman(spin_system, beta1_dense), beta1_zeeman))
        self.assertTrue(np.allclose(states.rho_to_zeeman(spin_system, beta2_dense), beta2_zeeman))
    ...
    def test_singlet(self):
        ...
        singlet_sparse = states.singlet(spin_system, 0,1, sparse=True)
        singlet_dense = states.singlet(spin_system, 0,1, sparse=False)
        self.assertTrue(np.allclose(singlet_zeeman, states.rho_to_zeeman(spin_system, singlet_sparse)))
        self.assertTrue(np.allclose(singlet_zeeman, states.rho_to_zeeman(spin_system, singlet_dense)))
    ...
    def test_triplet_zero(self):
        ...
        triplet_zero_sparse = states.triplet_zero(spin_system, 0,1, sparse=True)
        triplet_zero_dense = states.triplet_zero(spin_system, 0,1, sparse=False)
        self.assertTrue(np.allclose(triplet_zero_zeeman, states.rho_to_zeeman(spin_system, triplet_zero_sparse)))
        self.assertTrue(np.allclose(triplet_zero_zeeman, states.rho_to_zeeman(spin_system, triplet_zero_dense)))
    ...
    def test_triplet_plus(self):
        ...
        triplet_plus_sparse = states.triplet_plus(spin_system, 0,1, sparse=True)
        triplet_plus_dense = states.triplet_plus(spin_system, 0,1, sparse=False)
        self.assertTrue(np.allclose(triplet_plus_zeeman, states.rho_to_zeeman(spin_system, triplet_plus_sparse)))
        self.assertTrue(np.allclose(triplet_plus_zeeman, states.rho_to_zeeman(spin_system, triplet_plus_dense)))
    ...
    def test_triplet_minus(self):
        ...
        triplet_minus_sparse = states.triplet_minus(spin_system, 0,1, sparse=True)
        triplet_minus_dense = states.triplet_minus(spin_system, 0,1, sparse=False)
        self.assertTrue(np.allclose(triplet_minus_zeeman, states.rho_to_zeeman(spin_system, triplet_minus_sparse)))
        self.assertTrue(np.allclose(triplet_minus_zeeman, states.rho_to_zeeman(spin_system, triplet_minus_dense)))
        ...
        for i in test_states:
            for j in test_states:
                for k in test_states:
                    ...
                    state_sparse = states.state(spin_system, (i, j, k), (0, 1, 2), sparse=True)
                    state_dense = states.state(spin_system, (i, j, k), (0, 1, 2), sparse=False)
                    ...
                    self.assertTrue(np.allclose(states.rho_to_zeeman(spin_system, state_sparse), state_zeeman))
                    self.assertTrue(np.allclose(states.rho_to_zeeman(spin_system, state_dense), state_zeeman))
    ...
    def test_unit_state(self):
        ...
        unit_sparse = states.unit_state(spin_system, sparse=True, normalized=False)
        unit_dense = states.unit_state(spin_system, sparse=False, normalized=False)

        # Compare
        self.assertTrue(np.allclose(states.rho_to_zeeman(spin_system, unit_sparse), unit_zeeman))
        self.assertTrue(np.allclose(states.rho_to_zeeman(spin_system, unit_dense), unit_zeeman))

        # Create the trace-normalized unit state in spherical tensor basis
        unit_sparse = states.unit_state(spin_system, sparse=True, normalized=True)
        unit_dense = states.unit_state(spin_system, sparse=False, normalized=True)
                    ...
                    for l in test_states:
                        for m in test_states:
                            for n in test_states:
                                ...
                                result_sparse = states.measure(spin_system, state_sparse, (l, m, n), (0, 1, 2))
                                result_dense = states.measure(spin_system, state_dense, (l, m, n), (0, 1, 2))
    ...
    def test_thermal_equilibrium(self):
        ...
        rho_eq_sparse = states.thermal_equilibrium(spin_system, temperature, field, sparse=True)
        rho_eq_dense = states.thermal_equilibrium(spin_system, temperature, field, sparse=False)

        # Test the thermal equilibrium for each spin
        for i in range(size):

            # Measure the magnetization
            Mz_measured_sparse = states.measure(spin_system, rho_eq_sparse, 'I_z', i)
            Mz_measured_dense = states.measure(spin_system, rho_eq_dense, 'I_z', i)
