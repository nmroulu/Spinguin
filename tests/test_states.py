import unittest
import numpy as np
from spinguin.qm.operators import op_E, op_Sx, op_Sy, op_Sz, op_Sp, op_Sm
from spinguin.qm.states import alpha_state, beta_state, state_to_zeeman, singlet_state, \
...
from spinguin.qm.hamiltonian import sop_H_Z
...
class TestStates(unittest.TestCase):
    ...
    def test_alpha(self):
        ...
        # Compare
        self.assertTrue(np.allclose(state_to_zeeman(basis, spins, alpha1_sparse, sparse=False), alpha1_zeeman))
        self.assertTrue(np.allclose(state_to_zeeman(basis, spins, alpha2_sparse, sparse=False), alpha2_zeeman))
        self.assertTrue(np.allclose(state_to_zeeman(basis, spins, alpha1_dense, sparse=False), alpha1_zeeman))
        self.assertTrue(np.allclose(state_to_zeeman(basis, spins, alpha2_dense, sparse=False), alpha2_zeeman))
    ...
    def test_beta(self):
        ...
        # Compare
        self.assertTrue(np.allclose(state_to_zeeman(basis, spins, beta1_sparse, sparse=False), beta1_zeeman))
        self.assertTrue(np.allclose(state_to_zeeman(basis, spins, beta2_sparse, sparse=False), beta2_zeeman))
        self.assertTrue(np.allclose(state_to_zeeman(basis, spins, beta1_dense, sparse=False), beta1_zeeman))
        self.assertTrue(np.allclose(state_to_zeeman(basis, spins, beta2_dense, sparse=False), beta2_zeeman))
    ...
    def test_singlet(self):
        ...
        self.assertTrue(np.allclose(singlet_zeeman, state_to_zeeman(basis, spins, singlet_sparse, sparse=False)))
        self.assertTrue(np.allclose(singlet_zeeman, state_to_zeeman(basis, spins, singlet_dense, sparse=False)))
    ...
    def test_triplet_zero(self):
        ...
        triplet_zero_sparse = triplet_zero_state(basis, spins, 0, 1, sparse=True)
        triplet_zero_dense = triplet_zero_state(basis, spins, 0, 1, sparse=False)
        self.assertTrue(np.allclose(triplet_zero_zeeman, state_to_zeeman(basis, spins, triplet_zero_sparse, sparse=False)))
        self.assertTrue(np.allclose(triplet_zero_zeeman, state_to_zeeman(basis, spins, triplet_zero_dense, sparse=False)))
    ...
    def test_triplet_plus(self):
        ...
        triplet_plus_sparse = triplet_plus_state(basis, spins, 0, 1, sparse=True)
        triplet_plus_dense = triplet_plus_state(basis, spins, 0, 1, sparse=False)
        self.assertTrue(np.allclose(triplet_plus_zeeman, state_to_zeeman(basis, spins, triplet_plus_sparse, sparse=False)))
        self.assertTrue(np.allclose(triplet_plus_zeeman, state_to_zeeman(basis, spins, triplet_plus_dense, sparse=False)))
    ...
    def test_triplet_minus(self):
        ...
        triplet_minus_sparse = triplet_minus_state(basis, spins, 0, 1, sparse=True)
        triplet_minus_dense = triplet_minus_state(basis, spins, 0, 1, sparse=False)
        self.assertTrue(np.allclose(triplet_minus_zeeman, state_to_zeeman(basis, spins, triplet_minus_sparse, sparse=False)))
        self.assertTrue(np.allclose(triplet_minus_zeeman, state_to_zeeman(basis, spins, triplet_minus_dense, sparse=False)))
        ...
        for i in test_states:
            ...
            for j in test_states:
                ...
                for k in test_states:
                    ...
                    state_sparse = state_from_string(basis, spins, op_string, sparse=True)
                    state_dense = state_from_string(basis, spins, op_string, sparse=False)
                    ...
                    state_sparse_to_zeeman_dense = state_to_zeeman(basis, spins, state_sparse, sparse=False)
                    state_sparse_to_zeeman_sparse = state_to_zeeman(basis, spins, state_sparse, sparse=True).toarray()
                    state_dense_to_zeeman_dense = state_to_zeeman(basis, spins, state_dense, sparse=False)
                    state_dense_to_zeeman_sparse = state_to_zeeman(basis, spins, state_dense, sparse=True).toarray()

                    # Compare to the Zeeman reference
                    self.assertTrue(np.allclose(state_sparse_to_zeeman_dense, state_zeeman))
                    self.assertTrue(np.allclose(state_sparse_to_zeeman_sparse, state_zeeman))
                    self.assertTrue(np.allclose(state_dense_to_zeeman_dense, state_zeeman))
                    self.assertTrue(np.allclose(state_dense_to_zeeman_sparse, state_zeeman))
    ...
    def test_unit_state(self):
        ...
        # Compare
        self.assertTrue(np.allclose(state_to_zeeman(basis, spins, unit_sparse, sparse=False), unit_zeeman))
        self.assertTrue(np.allclose(state_to_zeeman(basis, spins, unit_dense, sparse=False), unit_zeeman))
                    ...
                    for l in test_states:
                        ...
                        for m in test_states:
                            ...
                            for n in test_states:
                                ...
                                result_sparse = measure(basis, spins, state_sparse, op_string)
                                result_dense = measure(basis, spins, state_dense, op_string)
    ...
    def test_equilibrium_state(self):
        ...
        state against the reference value calculated from Boltzmann distribution.
        """
        ...
        rho_eq_sparse = equilibrium_state(basis, spins, H_left, T, sparse=True, zero_value=1e-18)
        rho_eq_dense = equilibrium_state(basis, spins, H_left, T, sparse=False, zero_value=1e-18)

        # Test the thermal equilibrium for each spin
        for i in range(nspins):

            # Measure the z-magnetization
            Mz_measured_sparse = measure(basis, spins, rho_eq_sparse, f"I(z, {i})")
            Mz_measured_dense = measure(basis, spins, rho_eq_dense, f"I(z, {i})")
