...
class TestChemMethods(unittest.TestCase):
    ...
    def test_associate(self):
        ...
        rho1 = _states.alpha_state(ss1, 0)
        rho2 = _states.alpha_state(ss2, 0)

        # Perform association
        rho3 = _chem.associate(ss1, ss2, ss3, rho1, rho2, spinmap1, spinmap2)

        # Create the expected state directly and compare
        self.assertTrue(np.allclose(rho3, _states.triplet_plus_state(ss3, 0, 2)))
    ...
    def test_dissociate(self):
        ...
        rho3 = _states.triplet_plus_state(ss3, 0, 2)
        ...
        self.assertTrue(np.allclose(rho1, _states.alpha_state(ss1, 0)))
        self.assertTrue(np.allclose(rho2, _states.alpha_state(ss2, 0)))
    ...
    def test_permute_spins(self):
        ...
        rho = _states.alpha_state(ss, 0)

        # Compare the results of rotations with expected states
        self.assertTrue((_chem.permute_spins(ss, rho, (0, 1, 2)) == _states.alpha_state(ss, 0)).all())
        self.assertTrue((_chem.permute_spins(ss, rho, (0, 2, 1)) == _states.alpha_state(ss, 0)).all())
        self.assertTrue((_chem.permute_spins(ss, rho, (1, 0, 2)) == _states.alpha_state(ss, 1)).all())
        self.assertTrue((_chem.permute_spins(ss, rho, (1, 2, 0)) == _states.alpha_state(ss, 1)).all())
        self.assertTrue((_chem.permute_spins(ss, rho, (2, 0, 1)) == _states.alpha_state(ss, 2)).all())
        self.assertTrue((_chem.permute_spins(ss, rho, (2, 1, 0)) == _states.alpha_state(ss, 2)).all())
