...
from spinguin.qm.relaxation import sop_R_redfield, sop_R_sr2k, ldb_thermalization, dd_constant
from spinguin.qm.propagation import sop_pulse, sop_propagator
from spinguin.utils.nmr_isotopes import ISOTOPES
from spinguin.qm.basis import make_basis, truncate_basis_by_coherence
from spinguin.qm.states import equilibrium_state, state_to_truncated_basis
from spinguin.qm.superoperators import sop_to_truncated_basis
...
class TestRelaxation(unittest.TestCase):
    ...
    def test_thermalization(self):
        ...
        # Get the Hamiltonian
        H = sop_H_coherent(basis, gammas, spins, chemical_shifts, J_couplings, B, side="comm", sparse=True, zero_value=1e-12)
        
        # Get the relaxation superoperator components
        R_redfield = sop_R_redfield(basis, H, tau_c, spins, B, gammas, quad, xyz, shielding, efg,
                           include_antisymmetric=False, include_dynamic_frequency_shift=False,
                           relative_error=1e-6, interaction_zero=1e-9, relaxation_zero=1e-12, sparse=True)
        R_sr2k = sop_R_sr2k(basis, spins, gammas, chemical_shifts, J_couplings, R_redfield, B, sparse=True)

        # Built the total relaxation superoperator and thermalize
        R = R_redfield + R_sr2k
        H_left = sop_H_coherent(basis, gammas, spins, chemical_shifts, J_couplings, B, side="left", sparse=True, zero_value=1e-12)
        ...
        rho = equilibrium_state(basis, spins, H_left, T, sparse=False, zero_value=1e-18)

        # Apply a 180-degree pulse (to all 1H)
        op_string = "I(x,0) + I(x,1) + I(x,2) + I(x,3) + I(x,4) + I(x,5)"
        pul_180 = sop_pulse(basis, spins, op_string, angle=180, sparse=True, zero_value=1e-18)
        rho = pul_180 @ rho

        # Switch to the zero-quantum (ZQ) subspace
        ZQ_basis, ZQ_map = truncate_basis_by_coherence(basis, coherence_orders=[0])
        L = sop_to_truncated_basis(ZQ_map, L)
        rho = state_to_truncated_basis(ZQ_map, rho)

        # Get the propagator
        P = sop_propagator(L, time_step, custom_dot=False, zero_value=1e-18, density_threshold=0.5)
        ...
        H_left_ZQ = sop_H_coherent(ZQ_basis, gammas, spins, chemical_shifts, J_couplings, B, side="left", sparse=True, zero_value=1e-12)
        rho_ref = equilibrium_state(ZQ_basis, spins, H_left_ZQ, T, sparse=False, zero_value=1e-18)
