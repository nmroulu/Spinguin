...
class TestNMRIsotopes(unittest.TestCase):
    ...
    def test_spin(self):
        ...
        self.assertEqual(sg.spin("1H"), 1/2)
        self.assertEqual(sg.spin("14N"), 1)
        self.assertEqual(sg.spin("19F"), 1/2)
        self.assertEqual(sg.spin("23Na"), 3/2)
    ...
    def test_gamma(self):
        ...
        # Compare against tabulated reference values.
        self.assertEqual(sg.gamma("1H", "Hz"), 42.577478615342585e6)
        self.assertEqual(sg.gamma("1H", "rad/s"), 2*np.pi*42.577478615342585e6)
        self.assertEqual(sg.gamma("14N", "Hz"), 3.076272817251739e6)
        self.assertEqual(sg.gamma("14N", "rad/s"), 2*np.pi*3.076272817251739e6)
        self.assertEqual(sg.gamma("19F", "Hz"), 40.06924371705693e6)
        self.assertEqual(sg.gamma("19F", "rad/s"), 2*np.pi*40.06924371705693e6)
        self.assertEqual(sg.gamma("23Na", "Hz"), 11.268733657034751e6)
        self.assertEqual(sg.gamma("23Na", "rad/s"), 2*np.pi*11.268733657034751e6)
    ...
    def test_quadrupole_moment(self):
        ...
        # Compare against tabulated reference values.
        self.assertEqual(sg.quadrupole_moment("1H"), 0)
        self.assertEqual(sg.quadrupole_moment("19F"), 0)
        self.assertEqual(sg.quadrupole_moment("14N"), 0.02044e-28)
        self.assertEqual(sg.quadrupole_moment("23Na"), 0.104e-28)
