"""
Update the files used for documentation version selection.

This script scans the gh-pages branch, finds the generated documentation
version numbers, sorts them, and creates the version_switcher.json file.
In addition, it creates the index.html file that redirects the user to the
most recent stable documentation version.
"""
...
def update_versions():
    # Obtain all directories from the root of gh-pages.
    folders = [item for item in os.listdir(".") if os.path.isdir(item)]

    # Collect the release numbers from the directories.
    latest_exists = False
    releases = []
    ...
    # Check that at least one release directory was found.
    if len(releases) == 0:
        raise ValueError("Could not find any releases.")

    # Sort first by patch, then by minor, and finally by major.
    ...
    # Initialise the JSON data structure.
    json_data = []

    # Add the latest documentation entry if it exists.
    if latest_exists:
        json_data.append({
            "version": "latest",
    ...
    # Add all stable releases to the JSON data.
    ...
    # Write the version_switcher.json file.
    with open(f"{BASE_FOLDER}/version_switcher.json", "w") as file:
        json.dump(json_data, file, indent=2)

    # Create the redirecting index.html file.
